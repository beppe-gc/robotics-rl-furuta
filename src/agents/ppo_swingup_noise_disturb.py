...
class Args:
    ...
    """whether to save model into the `furuta_pendulum_tensorboard/{run_name}` folder"""
...
def make_env(
    ...
    disturbance_steps=None,
    disturbance_amplitude=0.0,
):
    def thunk():
        env = gym.make(
            'FurutaPendulumSwingup-v0',
...
if __name__ == "__main__":
    ...
    run_name = f"furutaTorqueSwingup__{args.exp_name}__{args.seed}__{int(time.time())}"
    
    writer = SummaryWriter(f"furuta_pendulum_swingup_tensorboard/{run_name}")
    writer.add_text(
        "hyperparameters",
    ...
    envs = gym.vector.SyncVectorEnv([
        make_env(
            ...
            parameters_model=parameters_model,
            forward_dynamics_casadi_path=forward_dynamics_casadi_path,
            swingup=args.swingup,
            render=False,
            apply_noise=True,
            torque_noise_std=0.01,
            obs_noise_std=0.02,
            apply_disturbance=True,
            disturbance_steps=[100, 200, 300],
    ...
    if args.save_model:
        model_path = f"furuta_pendulum_swingup_tensorboard/{run_name}/{args.exp_name}.cleanrl_model"

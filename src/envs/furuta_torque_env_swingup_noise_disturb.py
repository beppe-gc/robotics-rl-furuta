...
class FurutaPendulumSimulator:
    def __init__(self, urdf_model_path, forward_dynamics_casadi_path, render=False, swingup=True, 
             apply_noise=False, torque_noise_std=0.0, obs_noise_std=0.0,
             apply_disturbance=False, disturbance_steps=None, disturbance_amplitude=0.0):
        self.render = render
        self.t = 0
        ...
        self.disturbance_steps = disturbance_steps or []
...
class FurutaPendulumSwingupEnv(gym.Env):
    def __init__(self, urdf_model_path, forward_dynamics_casadi_path, parameters_model,
             render=False, swingup=True,
             apply_noise=False, torque_noise_std=0.0, obs_noise_std=0.0,
             apply_disturbance=False, disturbance_steps=None, disturbance_amplitude=0.0):


        self.pendulum_sim = FurutaPendulumSimulator(
            ...
            disturbance_steps=disturbance_steps or [],
        ...
        self.time_limit = 5000  # maximum number of steps per episode
    ...
    def step(self, action):
        # Compute base torque from action
        torque = np.clip(action[0] * self._max_torque_joint0, -self._max_torque_joint0, self._max_torque_joint0)

        # Add disturbance if scheduled
        if self.pendulum_sim.apply_disturbance and self.time_step in self.pendulum_sim.disturbance_steps:
            torque += self.pendulum_sim.disturbance_amplitude
            print(f"[DEBUG] Disturbance applied at step {self.time_step}: +{self.pendulum_sim.disturbance_amplitude}")

        # Add torque noise
        if self.pendulum_sim.apply_noise:
            torque += np.random.normal(0, self.pendulum_sim.torque_noise_std)

        # Apply the (possibly disturbed and noisy) torque to the simulator
        self.pendulum_sim.step(torque, self.dt)
        ...
        if self.pendulum_sim.apply_noise:
            obs += np.random.normal(0, self.pendulum_sim.obs_noise_std, size=obs.shape)
        ...
        if (
            np.abs(self.qvel[0]) > self._max_velocity_joint0 or 
            np.abs(self.qvel[1]) > self._max_velocity_joint1 or 
            np.abs(self.qpos[0]) > self._max_angle_joint0
        ):
            terminated = True
            reward = -100
        elif not self.swingup and np.abs(self.qpos[1] - np.pi) > 45 * np.pi / 180:
            terminated = True
            reward = -100
        else:
            terminated = False
